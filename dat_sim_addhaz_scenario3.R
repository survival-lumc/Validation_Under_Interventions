...
#
#SCENARIO 3: Data are generated under scenario 1. At the end, we add random error into the L variables. 
#------------------------------
#------------------------------
...
error.var<-16 #16 means that the error variance is same as the variance of L
L.baseline.dat=dat.long$L[dat.long$visit==1]+rnorm(n,0,sqrt(error.var)) 

#in case you also want to add to the L that is used during weights estimation
#
...
#dat.long$L[dat.long$visit==5]<-dat.long$L[dat.long$visit==5]+rnorm(n,0,sqrt(8))
